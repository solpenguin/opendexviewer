...
const tokenDetail = {
  ...
  updateCommunitySection() {
    ...
    const defaultLinksWrapper = document.getElementById('default-links-wrapper');
    const defaultLinksContainer = document.getElementById('default-links');
    ...
    // Check for default metadata links
    const defaultLinks = this.token?.defaultLinks || null;
    const hasDefaultLinks = defaultLinks && Object.keys(defaultLinks).length > 0;

    // If neither banner, community links, nor default links — hide section
    if (!hasBanner && !hasLinks && !hasDefaultLinks) {
    ...
    // Handle links display — community links take priority over default metadata links
    if (hasLinks) {
      // Community links exist — show them, hide default links
      const grouped = {};
      ...
      linksContainer.innerHTML = Object.values(grouped).map(s => `
        <a href="${(s.content_url && (s.content_url.startsWith('https://') || s.content_url.startsWith('http://'))) ? this.escapeHtml(s.content_url) : '#'}" target="_blank" rel="noopener noreferrer" class="community-link-chip ${this.escapeHtml(s.submission_type)}">
          ${linkIcons[s.submission_type] || ''}
          <span>${linkLabels[s.submission_type] || this.escapeHtml(s.submission_type)}</span>
        </a>
      `).join('');
      linksContainer.style.display = 'flex';
      if (defaultLinksWrapper) defaultLinksWrapper.style.display = 'none';
    } else if (hasDefaultLinks) {
      // No community links — show default metadata links with warning
      linksContainer.style.display = 'none';
      if (defaultLinksWrapper && defaultLinksContainer) {
        defaultLinksContainer.innerHTML = Object.entries(defaultLinks).map(([type, url]) => {
          const safeUrl = (url && (url.startsWith('https://') || url.startsWith('http://'))) ? this.escapeHtml(url) : '#';
          return `
            <a href="${safeUrl}" target="_blank" rel="noopener noreferrer" class="community-link-chip ${this.escapeHtml(type)}">
              ${linkIcons[type] || ''}
              <span>${linkLabels[type] || this.escapeHtml(type)}</span>
            </a>
          `;
        }).join('');
        defaultLinksWrapper.style.display = 'block';
      }
    } else {
      linksContainer.style.display = 'none';
      if (defaultLinksWrapper) defaultLinksWrapper.style.display = 'none';
