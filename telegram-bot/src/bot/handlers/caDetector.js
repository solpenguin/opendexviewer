...
module.exports = (bot) => {
  bot.hears(SOLANA_CA_REGEX, async (ctx) => {
    // Always auto-detect in private chats
    if (ctx.chat.type === 'private') {
      return handleLookup(ctx);
    }

    // In groups, only auto-detect if the group has opted in
    if (ctx.chat.type === 'group' || ctx.chat.type === 'supergroup') {
      const setting = await alertStore.getGroupSetting(ctx.chat.id);
      if (setting.ca_detect) {
        return handleLookup(ctx);
      }
    }
  });
...
async function handleLookup(ctx) {
